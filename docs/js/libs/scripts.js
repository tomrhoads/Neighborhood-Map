...
viewModel= function() {
  ...
  /*this.visiblePlaces = ko.computed(function() {
    ...
    return this.parkLocations().query(function(place) {
      if(!self.query() || place.name.toLowerCase().indexOf(self.query().toLowerCase()) !== -1) {
        return place;
      };
    });
var initialParkLocations = [{
...
function initMap() {
  var mapOptions = {
    zoom: 13,
    center: {lat: 38.237869, lng: -122.666}
  };
  map = new google.maps.Map(document.getElementById('map-canvas'), mapOptions);
  ko.applyBindings(new viewModel());
  console.log("initMap");
...
viewModel= function() {
  ...
  initialParkLocations.forEach(function(parkItem) {
      ...
      var park = new ParkLocation(parkItem);
      if(!self.query() || parkItem.name.toLowerCase().indexOf(self.query().toLowerCase()) != -1){
      console.log("search filter if statement true");
      marker = new google.maps.Marker({
        position: parkItem.latlng,
        map: map,
